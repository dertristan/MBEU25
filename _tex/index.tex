...
(\citeproc{ref-neyman1923application}{Splawa-Neyman 1990};
...
\(\{Y_{ric}^{g}(1),\, Y_{ric}^{g}(0)\}  \perp T_{ric}\).\footnote{Table~\ref{tbl-balance-round-mods}
  ...
  consistent with random assignment of the religion attribute holding in
  the realized sample. Identification further relies on the usual
  conjoint stability assumptions of no carryover across rounds and no
...
population:\footnote{All quantities we report are sample quantities. We
  return to the implications for external validity in
...
respondent-level characteristics. Disaggregating to the individual level
by averaging over rounds, we define the respondent-level CATE:
...
Because these characteristics are not randomized, \(\tau_{ic}^{g}\)
describes how the penalty varies across subgroups rather than the causal
effect of changing a moderator
(\citeproc{ref-bansak2021estimating}{Bansak 2021}).
...
CATE:\footnote{The subscripts \(r, i, c\) reflect the hierarchical
  ...
  marginalizing over respondents. \(\mathbb{E}_r\) denotes expectation
  over the design distribution of the displayed attributes, which
  randomization makes identical across respondents. \(\mathbb{E}_i\)
  denotes averaging over the \(N\) sampled respondents, with \(N\) as in
  Equation~\ref{eq-sate}.}
...
(\citeproc{ref-hahn2020bayesian}{Hahn et al. 2020};
\citeproc{ref-thal2024aggregate}{Thal et al. 2024}). The sample
splitting behind the CF's confidence intervals is exactly that missing
device, and it addresses the concern our randomized design does not
dissolve.
...
respondents are themselves grouped within 25 countries, so the
observations are clustered at two levels. Treating them as independent
would understate our uncertainty. The CF accommodates this structure
directly and scales to the full sample. We cluster the sample splitting
and the variance estimation by respondent, which absorbs the
within-respondent dependence across rounds, and we enter country as
fixed effects in the covariate matrix, which absorbs between-country
baseline variation (\citeproc{ref-athey2019generalized}{Athey et al.
2019}).\footnote{Any residual within-country dependence beyond these
  fixed effects is not reflected in the variance estimates.} We
...
the CF behaves as it should before we ask it about
heterogeneity.\footnote{The BCF posterior means for the same quantity
  agree with these estimates.} We then gauge that heterogeneity with the
targeting operator characteristic curve
...
forest's own out-of-bag predictions rather than by a rule estimated on
held-out data, we read it as descriptive support for targetable
variation rather than a formal test, and it motivates the
...
than a partial effect with everything else held fixed. For categorical
...
ribbon, which lets the CF reveal any non-linear trend. Throughout, we
orient every rating scale so that moving right along the x-axis denotes
more agreement, a more positive evaluation, or more attachment,
depending on the item, with full wordings and original codings given in
Table~\ref{tbl-wordings}. We overlay the two games within a single panel
by color, so that any divergence between them reads directly as a
game-specific difference in heterogeneity, and because the games share
respondents and the moderator levels are many we read interval
comparisons across panels and games as descriptive rather than as formal
tests.
...
stable, since every level of every attribute carries a sizeable and
clearly negative effect and none of the accompanying features comes
close to neutralizing it. The clearest moderation is partisanship.
...
Among co-nationals, displaying a party affiliation reduces the penalty
substantially relative to withholding it, the single largest shift in
the figure, and the attenuation holds in both games. Nationality is the
only attribute whose moderation differs across the two games. The
penalty against non-EU national profiles is deeper in the Trust game
than in the Dictator game, the two intervals being disjoint. Within the
Dictator game the ordering is itself informative, as non-EU nationals
draw the smallest penalty while co-nationals are penalized
systematically more, an ordering the Trust game does not reproduce.
Social class moves the penalty only at its upper end, where lower- and
middle-class profiles are penalized indistinguishably but profiles
presented as upper class are penalized somewhat less. The remaining
attributes leave the penalty essentially untouched. Whether a profile
identifies as an EU citizen or rejects that identity makes no
difference, though in the Dictator game both attract a marginally
smaller penalty than the condition in which EU identity is not
displayed, a gap that lies within sampling error and does not recur in
the Trust game. Profile age produces only a minor, non-systematic dip at
...
respondent-level characteristics that govern who penalizes more, which
the introduction flagged as the substantively more consequential source
of variation. These characteristics display more heterogeneity than the
profile attributes above. Among the categorical respondent
characteristics, the sharpest contrast is by gender, as men penalize
Muslim profiles more than women in both games, though only in the
Dictator game do the intervals separate. Respondents' own religion
orders the penalty as well, with Catholics and Protestants exhibiting
the strongest bias and the Orthodox the weakest by their point
estimates. Place of residence matters at the margin, rural respondents
penalizing somewhat more than those in large cities and smaller towns
falling in between, a gradient that is firmer in the Trust game.
Subjective class is largely flat, its main features being the wide
intervals and smallest penalty of respondents who decline to place
themselves and a slightly but not systematically lower penalty among the
working class compared to lower-middle and middle class respondents in
the Dictator game. The upper middle class is the interesting exception,
drawing the smallest penalty of any class in the Dictator game but the
largest in the Trust game by their point estimates, the clearest
between-game divergence in the figure.
...
country runs the other way and appears to deepen the penalty, though the
scarcity of respondents at the unattached end leaves that pattern
uncertain. Attachment to Europe as a whole is weaker and less clear,
consistent with a faintly declining penalty in the Dictator game and a
flat one in the Trust game, while perceived ability to understand EU
politics shows no moderation at all. A second cluster concerns populist
and democratic dispositions. Agreement that officials merely talk rather
than act, and agreement that compromise amounts to selling out, are each
associated with a larger penalty, the latter tracing a clear monotonic
gradient. The view that politics is a struggle between good and evil
follows the same direction from the middle of the scale upward before
flattening near the top. Valuing compromise, its mirror image, is
associated with a marginally smaller penalty, although agreement is high
throughout the sample and few respondents populate the low end of that
item. Respondent age closes the figure and hints at a U-shaped
relationship. Among the youngest respondents the intervals brush the
zero line repeatedly, and at the very youngest ages the point estimates
briefly cross it, the only place in the analysis where the penalty
appears to vanish. The penalty then deepens through middle age and lifts
again past sixty-five, though that upturn coincides with the thinning of
the sample at older ages and may be an artifact of the wider intervals
there.
...
and those who judge it favorably penalize them less. More negative
assessments of the EU's effect on the national economy, on national
culture and identity, and on national political status each accompany a
larger penalty, as does agreement that the Union would be better
abolished and that it should heed the people. Satisfaction with the EU
and support for its pursuing long-term goals run the other way toward a
smaller penalty, the latter flattening at the top of its scale. The
technocratic and remaining populist items move the penalty far less.
Leaving decisions to experts and elevating leaders above ordinary
citizens are associated with at most a slight increase in the penalty as
agreement rises, more visible in the Trust game, and agreement that
parties do more harm than good nudges it upward in the same way. Valuing
the views of other groups behaves like the compromise item, edging the
penalty down over the upper half of the scale. The weakest moderators
are the claim that ordinary people do not know what is good for them and
the age at which respondents left education, both of which leave the
penalty essentially flat.

Taken together, the respondent-level results describe substantial
heterogeneity across demographic subgroups and pre-treatment attitudes,
but heterogeneity almost entirely in the magnitude of the Muslim penalty
rather than in its presence. In the few subgroups where the penalty
comes closest to disappearing, only the upper bound of the interval
grazes zero, and no single moderator credibly nullifies the penalty, let
alone reverses it. The strongest and most coherent moderation runs
through evaluations of European integration, where respondents hostile
to the Union or skeptical of its effects penalize Muslim profiles
systematically more than those who regard it favorably. Yet even on the
pro-European side of these scales a robust penalty remains, attenuated
rather than abolished. The variable-importance ranking in the appendix
...
and the same populist items track differences in its magnitude. The
continuous moderators trace smoother gradients under the BCF than under
the CF, yet their direction and shape are unchanged.
...
Two differences deserve comment, and both plausibly stem from the same
source. The BCF intervals are narrower than the CF's, most visibly at
sparsely populated moderator levels. A respondent cluster bootstrap
shows that the sampling variability contributed by within-person
dependence across the three rounds accounts for almost none of this
narrowness, which we therefore read as a consequence of the absence of
sample splitting and the aggressive pooling of the Bayesian prior rather
than as evidence of greater precision.\footnote{The bootstrap quantifies
  only one of the two channels through which the respondent structure
  enters. It resamples respondents and recomputes each bin's mean of the
  estimated effects, capturing the design and sampling variability of
  the displayed point estimates, and on the real fits this component is
...
patterns is left to future work. Relatedly, the respondent-level
moderators are observed rather than randomized, so the gradients
describe who penalizes more, not the effect of changing a moderator.
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-latex/code-09_additional_figs_tables-fig-outcome-distr-output-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-latex/code-07_postprocess_grf-fig-grf-combined-toc-output-1.png}}
