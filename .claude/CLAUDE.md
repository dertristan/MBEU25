...
1. **Nested random intercepts feasibility. RESOLVED (2026-06-06): nested fit infeasible; country-only REs adopted (the documented fallback).** Data has two clustering levels: observations nested in respondents nested in countries. The two-level nested extension is implemented in `multibart` and passes the synthetic smoke test (`03_multibart_nested_ri_test.qmd`, continuous path) — the math and machinery are correct. But the sampler forms the random-effects precision `Phi = adj·WtW·adj` densely and Cholesky-solves it every Gibbs sweep, an `O(q³)` cost in `q = C + R` dominated by the ~22k–37.5k respondent dummies. On a 10k-row real-data subsample this took >20 min to reach iteration 1; the cubic term makes the full-scale nested fit prohibitive in the project window. **Decision: drop the respondent RE and fit country random intercepts only** (`q ≈ 25`, the RE block is a trivial `C × C` solve), the fallback flagged here from the start. Respondent within-person dependence (3 conjoint tasks each) is handled by a **respondent cluster bootstrap** as a sensitivity check on the BCF side (the primary GRF/CRF side gets it free via `clusters = respondent_id`). The real-data path in `03_multibart_nested_ri_test.qmd` (`#sec-subsample`) now fits country-only REs and explains the cluster bootstrap (`#sec-cluster-bootstrap`); the synthetic section retains the full nested mechanism test (cheap at small scale). **Do not re-attempt the nested respondent-level fit** unless the sampler is rewritten to exploit the sparsity of `W`/`WtW` (a non-trivial C++ change, out of scope for this window). **Caveat on what the cluster bootstrap can and cannot do (2026-06-10):** the respondent cluster bootstrap quantifies only the *sampling* channel — it resamples respondents and recomputes each bin's mean of `tau_hat`, capturing design/sampling variability of the displayed point estimates. On the real fits this is ~an order of magnitude below `post_sd` (≈1–3% added to the bands), so within-person dependence does **not** explain the BCF's narrow intervals. But the bootstrap **cannot** correct the second channel: the BCF likelihood treats the ~84,600 rows as conditionally independent given the trees and the country random intercept, so any residual within-respondent correlation beyond what the country intercept absorbs makes the posterior over τ(x) concentrate on too much information and deflates `post_sd` internally (worst case, near-perfect within-respondent correlation, ≈√3 ≈ 1.7×). An additive bootstrap term cannot detect or repair that internal deflation. The honest read: the bootstrap bounds the sampling channel; the *likelihood* channel is gauged externally by the comparison with the honest, cluster-robust GRF, whose intervals come out wider. This is the right framing because BCF is robustness only — GRF/CRF is primary and gets clustering correct via `clusters = respondent_id`. Captured in the manuscript as footnote `[^bcf-clustering]` in `#sec-robustness`.
