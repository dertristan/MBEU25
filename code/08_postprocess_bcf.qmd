...
(`dont_know` last), the identity-scale reversal (`6 - x`, so higher means more attachment), the
two fixed display grids for the genuinely continuous moderators, and the number of cluster
bootstrap resamples.
...
level_keeps <- function(mm, mod) {
  ...
  if (is_num && length(unique(v)) > 12) {
    if (mod %in% names(cont_grids)) {
      ...
      out <- lapply(which(tabulate(cell, nbins = length(mids)) > 0), function(k) {
        keep <- !is.na(cell) & as.integer(cell) == k
        list(level = as.character(round(mids[k])), order = mids[k], is_scale = TRUE, keep = keep)
      })
      return(out)
    }
    bin <- ntile(v, 10)
    return(lapply(sort(unique(bin)), function(k) {
      ...
      list(
        level = as.character(round(mean(v[keep]))), order = mean(v[keep]),
        is_scale = TRUE, keep = keep
      )
    }))
  }
  ...
  out <- lapply(seq_along(levs), function(i) {
    lev <- levs[i]
    keep <- if (is_num) !is.na(v) & v == lev else !is.na(v) & as.character(v) == lev
    if (sum(keep) / sum(!is.na(v)) < 0.01) {
      return(NULL)
    }
    list(
      level = as.character(lev), order = if (is_num) as.numeric(lev) else i,
      is_scale = is_num && mod != "cj_age", keep = keep
    )
  })
...
cate_one_bcf <- function(tau_draws, mm, mod, game_label) {
  ...
  bind_rows(lapply(ks, function(L) {
    ...
    tibble(
      mod = mod, level = L$level, order = L$order, is_scale = L$is_scale,
      game = game_label, n = sum(L$keep),
      estimate = unname(p["estimate"]), post_sd = unname(p["post_sd"])
    )
  }))
}
```
...
make_Wb <- function(rid, B, seed) {
  ...
  R <- max(rid_int)
  Wb <- vapply(seq_len(B), function(b) tabulate(sample.int(R, R, replace = TRUE), nbins = R), integer(R))
...
m_dict <- game_mods(dat, "cj_dict") |>
  ...
  mutate(
    across(c(q_identity_country, q_identity_eu, q_identity_europe), flip5),
...
boot_prof_dict <- bind_rows(
  lapply(prof_mods, \(m) boot_sd_one(tau_hat_dict, bw_dict$rid_int, bw_dict$Wb, m_dict, m))
) |>
  mutate(game = "Dictator game")
boot_resp_dict <- bind_rows(
  lapply(resp_mods, \(m) boot_sd_one(tau_hat_dict, bw_dict$rid_int, bw_dict$Wb, m_dict, m))
) |>
  mutate(game = "Dictator game")
rm(bw_dict)
invisible(gc())
...
boot_prof_trust <- bind_rows(
  lapply(prof_mods, \(m) boot_sd_one(tau_hat_trust, bw_trust$rid_int, bw_trust$Wb, m_trust, m))
) |>
  mutate(game = "Trust game")
boot_resp_trust <- bind_rows(
  lapply(resp_mods, \(m) boot_sd_one(tau_hat_trust, bw_trust$rid_int, bw_trust$Wb, m_trust, m))
) |>
  mutate(game = "Trust game")
rm(bw_trust)
invisible(gc())
...
and `q_class` runs working class up to higher class with `dont_know` last.
...
The three identity scales are reversed so higher means more attachment, and `q_class` runs working
