...
The design `sub_X` is `model.matrix(~ . - 1, ...)`, the same column basis as GRF's `X_dict`, so the
terms are comparable.
The displayed-nationality and partisanship moderators are structurally collinear (partisanship is
shown or suppressed only for conational profiles, so its dummies sum exactly to the own-country
nationality dummy), leaving the design rank-deficient by one column.
We therefore drop the aliased column by QR pivoting before inverting `crossprod(sub_X)` — the same
identifiable basis grf's `best_linear_projection` uses internally — so no added intercept and no
ridge are needed.
...
proj_summary <- function(tau_draws, sub_X) {
  # drop linearly dependent columns via QR pivoting (matches grf's BLP)
  qrX <- qr(sub_X)
  X <- sub_X[, qrX$pivot[seq_len(qrX$rank)], drop = FALSE]
  xtx <- crossprod(X) # p x p
  m <- tau_draws %*% X # nsim x p  (= tau_d' X per draw)
