...
#| fig-cap: "BCF posterior estimates of the profile-level CATE of the Muslim profile across the levels of every conjoint-round moderator (x-axis), Dictator and Trust games overlaid and distinguished jointly by colour, shape, and linetype. The y-axis ($\\hat{\\tau}_{ric}$) is the profile-level CATE on the token-allocation scale. Points are per-level posterior medians and the vertical bars are cluster-robust 95% intervals, formed as the posterior median plus or minus 1.96 times a combined standard error (a normal approximation), where that standard error combines the posterior SD of the within-level mean of the CATE with a respondent cluster-bootstrap SD. Grey bars along the bottom of each facet show the per-level row count (pooled over games). The y-axis is free per facet. Levels comprising less than 1% of a game's sample are dropped because their CATE is too imprecise to compare. In the Profile partisanship facet, partisanship not shown refers to within the co-national category only, since partisanship is displayed for co-national profiles only."
#| dpi: 500
#| fig-width: 9
#| fig-height: 6
plot_comb(
