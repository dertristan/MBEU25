...
suppressed (`not_shown`, treatment 4 with the drawn nationality equal to the
respondent's country, i.e. `cj_nat == "own_country"`); every other profile is
`not_applicable`.
...
count(eu25_long, cj_treatment, cj_nat, der_cj_partisanship) |>
  kable(caption = "der_cj_partisanship by treatment condition and drawn nationality.")
```
...
stopifnot(
  ...
  all(
    (eu25_long$der_cj_partisanship == "not_shown") ==
      (eu25_long$cj_treatment == "4_somenat" & eu25_long$cj_nat == "own_country")
  )
)
```
